    ...
    if not type or type in ("expense", "all"):
        ...
        for e in q.all():
            ...
            items.append({
                ...
                "collected_at":  e.approved_at.isoformat() if e.approved_at else None,
    ...
    if not type or type in ("cash_submission", "all"):
        ...
        for cs in q.all():
            ...
            items.append({
                ...
                "method":        "cash",
                "created_at":    cs.approved_at.isoformat() if cs.approved_at else (cs.submitted_at.isoformat() if cs.submitted_at else None),
                "created_by":    "collector",
                "head_name":     collector_name,
                "head_id":       None,
                "chanda_no":     None,
                "collected_by":  admin_name,
                "collected_at":  cs.approved_at.isoformat() if cs.approved_at else None,
                "proof_image":   None,
                "transaction_ref": None,
                "notes":         cs.notes,
                "covered_months": [],
...
def get_collector_history(
    ...
    for d in donations:
        ...
        items.append({
            ...
            "created_at":             d.created_at.isoformat() if d.created_at else None,
            "collected_at":           d.donation_date.isoformat() if d.donation_date else (d.created_at.isoformat() if d.created_at else None),
            "verified_at":            None,
